...
def makeunicodedata(unicode, trace):
    ...
    for char in unicode.chars:
        ...
        if record:
            if record[5]:
                ...
                assert prefix < 256
                # content
                # XXX: check if all prefixes should be considered for compatibility decomposition
                ...
                try:
                    ...
                    decomp_utf16[0] = prefix + ((len(decomp_utf16)-1) << 8)
                    i = len(decomp_data)
    ...
    print("use db_types::*;", file=fp)
    print(file=fp)
    print('pub const UNIDATA_VERSION: &str = "%s\\0";' % UNIDATA_VERSION, file=fp)
    print(file=fp)
    print("/* a list of unique database records */", file=fp)
    print("pub static UCD_RECORDS: [UCDRecord; %d] = [" % len(table), file=fp)
    for item in table:
        print("    UCDRecord{category: %d, combining: %d, bidi_class: %d, mirrored: %d, east_asian_width: %d, script: %d, linebreak_class: %d}," % item, file=fp)
    print("];", file=fp)
    print(file=fp)

    #print("pub const BIDI_MIRROR_LEN: u32 = %d;" % len(unicode.bidi_mirroring), file=fp)
    print("pub static MIRROR_PAIRS: [MirrorPair; %d] = [" % len(unicode.bidi_mirroring), file=fp)
    for pair in unicode.bidi_mirroring:
        print("    MirrorPair{from: %d, to: %d}," % pair, file=fp)
    print("];", file=fp)
    print(file=fp)

    #print("pub const BIDI_BRACKET_LEN: u32 = %d;" % len(unicode.bidi_brackets), file=fp)
    print("pub static BRACKET_PAIRS: [BracketPair; %d] = [" % len(unicode.bidi_brackets), file=fp)
    for bracket in unicode.bidi_brackets:
        print("    BracketPair{from: %d, to: %d, bracket_type: %d}," % bracket, file=fp)
    print("];", file=fp)
    print(file=fp)

    print("/* Reindexing of NFC first characters. */", file=fp)
    print("#[allow(dead_code)] pub const TOTAL_FIRST: u32 = %d;" % total_first, file=fp)
    print("#[allow(dead_code)] pub const TOTAL_LAST: u32 = %d;" % total_last, file=fp)
    print("pub static NFC_FIRST: [ReIndex; %d] = [" % (len(comp_first_ranges)+1), file=fp)
    for start,end in comp_first_ranges:
        print("    ReIndex{start: %d, count: %d, index: %d}," % (start,end-start,comp_first[start]), file=fp)
    print("    ReIndex{start:0,count:0,index:0},", file=fp)
    print("];\n", file=fp)
    print("pub static NFC_LAST: [ReIndex; %d] = [" % (len(comp_last_ranges)+1), file=fp)
    for start,end in comp_last_ranges:
        print("    ReIndex{start: %d, count: %d, index: %d}," % (start,end-start,comp_last[start]), file=fp)
    print("    ReIndex{start:0,count:0,index:0},", file=fp)
    print("];\n", file=fp)
    ...
    print("#[allow(non_camel_case_types)] #[derive(Copy, Clone, PartialEq, Debug)] #[repr(u8)]", file=fp)
    print("pub enum EastAsianWidth {", file=fp)
    for name in EASTASIANWIDTH_NAMES:
        print("    %s = %d," % (name.upper(), EASTASIANWIDTH_NAMES.index(name)), file=fp)
    print("}", file=fp)
    print(file=fp)

    print("#[allow(non_camel_case_types)] #[derive(Copy, Clone, PartialEq, Debug)] #[repr(u8)]", file=fp)
    print("pub enum Script {", file=fp)
    for name in SCRIPT_NAMES:
        print("    %s = %d," % (name.upper(), SCRIPT_NAMES.index(name)), file=fp)
    print("}", file=fp)
    print(file=fp)
    ...
    print("#[allow(non_camel_case_types)] #[derive(Copy, Clone, PartialEq, Debug)] #[repr(u8)]", file=fp)
    print("pub enum GeneralCategory {", file=fp)
    for name in CATEGORY_NAMES:
        print("    %s = %d," % (name.upper(), CATEGORY_NAMES.index(name)), file=fp)
    print("}", file=fp)
    print(file=fp)

    print("#[allow(non_camel_case_types)] #[derive(Copy, Clone, PartialEq, Debug)] #[repr(u8)]", file=fp)
    print("pub enum BiDiClass {", file=fp)
    for name in BIDIRECTIONAL_NAMES:
        print("    %s = %d," % (name.upper(), BIDIRECTIONAL_NAMES.index(name)), file=fp)
    print("}", file=fp)
    print(file=fp)
    ...
    print("#[allow(dead_code)] pub const SHIFT1: u32 = %d;" % shift1, file=fp)
    print("#[allow(dead_code)] pub const SHIFT2: u32 = %d;" % shift2, file=fp)
    ...
    print("#[allow(dead_code)] pub const DECOMP_SHIFT1: u32 = %d;" % shift1, file=fp)
    print("#[allow(dead_code)] pub const DECOMP_SHIFT2: u32 = %d;" % shift2, file=fp)
    ...
    print("#[allow(dead_code)] pub const COMP_SHIFT1: u32 = %d;" % shift1, file=fp)
    print("#[allow(dead_code)] pub const COMP_SHIFT2: u32 = %d;" % shift2, file=fp)
...
class Array:
    ...
    def dump(self, file, trace=0):
        ...
        file.write("pub static ")
        file.write(self.name + ": ")
        if size == 1:
            file.write("[u8; %d]" % len(self.data))
        elif size == 2:
            file.write("[u16; %d]" %len(self.data))
        else:
            file.write("[u32; %d]" %len(self.data))
        file.write(" = [\n")
        if self.data:
            s = "    "
